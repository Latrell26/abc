...
export function mapApiResponseToAuditResult(
  ...
  // When no page produced a speed score, surface the most common
  // per-page PageSpeed reason so the dashboard can say *why* (-1 alone
  // reads like a bug). Usually all pages fail the same way.
  const psiErrors = successful
    .map((p) => p.psiError)
    .filter((e): e is string => typeof e === "string" && e.length > 0);
  const pageSpeedError =
    pageSpeedScore === -1 && psiErrors.length > 0
      ? mostCommon(psiErrors)
      : undefined;
  ...
  return {
    ...
    pages: api.pageResults.map((page): AuditPageSummary => {
      ...
      return {
        ...
        checks: pageChecks,
        error: page.error,
        psiError: page.psiError,
      };
    }),
  };
}

/** Most frequent value (first wins ties). Small helper, no dependency. */
